...
export const initDBForChat = async () => {
  await mongoose.connect(process.env.MONGODB_URL);
};

initDBForChat()
  .then(() => {
    chatbot("I am having cough and I want Azithromycin tablets");
  })
...
\`\`\``,
  ...
  for await (const chunk of response) {
    if (chunk.text) {
      ...
      console.log(chunk.functionCalls[0]);
      ...
      return res.status(200).json(final_response.text);
    }
  }
}
